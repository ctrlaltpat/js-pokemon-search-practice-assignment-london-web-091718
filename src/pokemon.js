class Pokemon {
  ...
  addPokemon() {
    ...
    container.id = `pokedex-${this.id}`;
    container.innerHTML = `
      <div class="pokemon-name">${this.name}</div>
      <div class="pokemon-img-holder">
        <img class="pokemon-img front" src="${this.sprites.front}" alt="${this.name}-front">
        <img class="pokemon-img back hide" src="${this.sprites.back}" alt="${this.name}-back">
      </div>
      <div class="types">
        ${this.types.map(t => "<div class=\"type " + t + "\" >" + t.toUpperCase() + "</div>").join("")}
      </div>
      <div class="details">
        <button class="stats-btn">Stats</button>
        <button class="abilities-btn">Abilities</button>
        <button class="moves-btn">Moves</button>
      </div>
    `;

    let statsEl = document.createElement("div");
    statsEl.className = "stats more-info hidden";
    statsEl.innerHTML = `
      <button>X</button>
      <ul>
        <li>HEIGHT : <span>${this.height}</span></li>
        <li>WEIGHT : <span>${this.weight}</span></li>
        <li>HP : <span>${this.stats["hp"]}</span></li>
        <li>ATTACK : <span>${this.stats["attack"]}</span></li>
        <li>DEFENSE : <span>${this.stats["defense"]}</span></li>
        <li>SPEED : <span>${this.stats["speed"]}</span></li>
        <li>SPECIAL ATTACK : <span>${this.stats["special-attack"]}</span></li>
        <li>SPECIAL DEFENSE : <span>${this.stats["special-defense"]}</span></li>
      </ul>
    `;

    let abilitiesEl = document.createElement("div");
    abilitiesEl.className = "abilities more-info hidden";
    let pokemonAbilities = this.abilities.map(ability => "<li>" + ability.toUpperCase() + "</li>").join("") || "NONE";
    ...
    movesEl.className = "moves more-info hidden";
    let pokemonMoves = this.moves.map(move => "<li>" + move.toUpperCase() + "</li>").join("") || "NONE";
    movesEl.innerHTML = `
      <button>X</button>
      <ul>
        ${pokemonMoves}
    ...
    container.querySelector('.stats-btn').addEventListener('click', this.toggleStats);
    container.querySelector('.abilities-btn').addEventListener('click', this.toggleAbilities);
    container.querySelector('.moves-btn').addEventListener('click', this.toggleMoves);
    statsEl.querySelector('button').addEventListener('click', this.toggleStats)
    abilitiesEl.querySelector('button').addEventListener('click', this.toggleAbilities)
    movesEl.querySelector('button').addEventListener('click', this.toggleMoves)
    container.appendChild(statsEl);
    container.appendChild(abilitiesEl);
    container.appendChild(movesEl);

    this.pokeList.appendChild(container);
  }

  toggleStats() {
    let stats = this.parentElement.parentElement.querySelector('.stats');
    stats.classList.toggle("hidden");
  }
  toggleAbilities() {
    let abilities = this.parentElement.parentElement.querySelector('.abilities');
    abilities.classList.toggle("hidden");
  }
  toggleMoves() {
    let moves = this.parentElement.parentElement.querySelector('.moves');
